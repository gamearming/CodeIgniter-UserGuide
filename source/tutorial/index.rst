...
- `靜態頁面 <static_pages>`_： 此節主要介紹 **控制器**、**檢視**、 **路由** 的基礎知識。(**Controllers**,**Views**,**Routing**)
- `新聞模組 <news_section>`_： 此節開始介紹 **模型** 的相關知識，並且執行簡單的資料庫操作。(**Models**)
- `新增新聞內容 <create_news_items>`_： 此節開始使用更進階的資料庫操作及表單驗證。
- `學習結論 <conclusion>`_： 總結整個教學，並指引出未來學習方向及其它資源連結。   
