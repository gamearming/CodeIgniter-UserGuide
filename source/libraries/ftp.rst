...
FTP 類還提供了一個 "鏡像" 功能，允許您將您本地的一個資料夾通過 FTP 整個的同步到遠程伺服器上。
...
.. php:class:: CI_FTP
	...
	.. php:method:: delete_dir($filepath)
		...
		用於刪除一個資料夾以及該資料夾下的所有文件。需要提供資料夾的路徑（以斜線結尾）。

		.. important:: 使用該成員函數要非常小心！
			它會遞歸的刪除資料夾下的所有內容，包括子資料夾和所有文件。請確保您提供的路徑是正確的。
	...
	.. php:method:: mirror($locpath, $rempath)
		...
		遞歸的讀取文字的一個資料夾和它下面的所有內容（包括子資料夾），然後通過 FTP 在遠程伺服器上建立一個鏡像。
		無論原文件的路徑和資料夾結構是什麼樣的，都會在遠程伺服器上一模一樣的重建。您需要指定一個原路徑和目的路徑::
	...
	.. php:method:: mkdir($path[, $permissions = NULL])
		...
		用於在伺服器上建立一個資料夾。需要指定資料夾的路徑並以斜線結尾。
